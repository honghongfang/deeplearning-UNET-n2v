...
parser.add_argument("--dims", help="dimensions of your data, can include: X,Y,Z,C (channel), T (time)", default='YXC')
...
parser.add_argument("--singleNetPerChannel",  help="if true - separate U-Net per channel", default=False, type=bool)
...
#from models.n2v_standard import N2V   #####
...
print(pshape)
patches = datagen.generate_patches_from_list(imgs[:1], shape=pshape, augment=False)
...
config = N2VConfig(X, unet_kern_size=args.netKernelSize,
                   train_steps_per_epoch=int(args.stepsPerEpoch),train_epochs=int(args.epochs), train_loss='combined', batch_norm=True,
                   ...
                   single_net_per_channel=args.singleNetPerChannel
                   )
